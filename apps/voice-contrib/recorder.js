 ...
 * Phase 2: auto-download audio, in-browser manifest accumulation
...
const Recorder = (() => {
  ...
  function init() {
    ...
    btnDownload.addEventListener("click", _saveContribution);
    btnManifest.addEventListener("click", _downloadManifest);
  ...
  function _buildYamlEntry(contribId, itemId) {
    ...
    let prov = "";
    if (meta) {
      prov += "    project: co_sense\n";
      prov += "    entity_id: " + (meta.entity_id || "") + "\n";
      prov += "    course: " + (meta.source_course || "") + "\n";
      if (meta.exp_id) {
  ...
  function _buildFullManifest() {
    var lines = ["# Contributions manifest", "# Generated by Voice Contributor", "# License: CC-BY-4.0", "contributions:"];
    _sessionEntries.forEach(function(e) { lines.push(e.yaml); });
    return lines.join("\n") + "\n";
  }
  ...
  function _updatePreview() {
    ...
    yamlPreview.textContent = _buildFullManifest();
    manifestCount.textContent = _sessionEntries.length + " item" + (_sessionEntries.length !== 1 ? "s" : "");
    btnManifest.style.display = "inline-block";
    recordCount.textContent = _sessionEntries.length + " saved";
  }
  ...
  async function _saveContribution() {
    ...
    _sessionEntries.push({ id: contribId, yaml: yamlEntry, ext: ext });
    _updatePreview();

    /* Try server-side save first, fall back to browser download */
    var saved = await _tryServerDonate(contribId, ext, yamlEntry);
    if (!saved) {
      const audioFilename = "audio/" + contribId + "." + ext;
      _triggerDownload(audioFilename, _recordedBlob);
    }

    _setStatus("Saved " + contribId + " (" + _sessionEntries.length + " total in session)", "success");
    btnNext.style.display = "inline-block";
  }

  async function _tryServerDonate(id, ext, yaml) {
    return new Promise(function(resolve) {
      ...
      reader.onload = function() {
        ...
        xhr.send(JSON.stringify({ id: id, audioBase64: audioBase64, yaml: yaml, ext: ext }));
      };
  ...
  function _downloadManifest() {
    var yaml = _buildFullManifest();
    var blob = new Blob([yaml], { type: "text/yaml;charset=utf-8" });
    _triggerDownload("contributions.yaml", blob);
    _setStatus("Manifest downloaded: " + _sessionEntries.length + " items", "success");
  }
